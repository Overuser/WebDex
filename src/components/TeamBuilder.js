...
const TeamBuilder = ({
  ...
  const { data: teams, isPending, error, setData: setTeams, setIsPending, setError } = useFetch('http://localhost:5000/teams')
              ...
              <div className="Teams">
                {
                  teams.map((team, i) => {
                    return (
                      <TeamBlock key={ i } dex={ team } index={ i } deleteTeam={ deleteTeam }/>
                    )
                  })
                }
              </div>
